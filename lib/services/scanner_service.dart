...
class ScannerService {
  static String _computeMd5(File file) {
  ...
  static Future<int> processMetadata(
    ...
    for (int i = 0; i < filePaths.length; i++) {
      ...
      try {
        ...
        final md5Hash = _computeMd5(file);
        final exists = await DatabaseService.md5Exists(md5Hash);
        if (exists) continue;

        final metadata = await _extractMetadata(filePath);
        final fileName = p.basenameWithoutExtension(filePath);
        final lrcExists = await _checkLrcExists(filePath);
  ...
  static Future<Map<String, String>> _extractMetadata(String filePath) async {
    ...
    try {
      ...
      if (_hasId3v2(bytes)) {
        final tagData = _parseId3v2(bytes);
        result.addAll(tagData);
      }
  ...
  static bool _hasId3v2(List<int> bytes) {
    if (bytes.length < 10) return false;
    return bytes[0] == 0x49 && bytes[1] == 0x44 && bytes[2] == 0x33;
  }

  static bool _hasId3v1(List<int> bytes) {
  ...
  static Map<String, String> _parseId3v2(List<int> bytes) {
    final result = <String, String>{};
    int offset = 0;
  ...
  static String _readLatin1(List<int> bytes) {
    return String.fromCharCodes(bytes.where((b) => b != 0));
  }

  static String _readUtf16LE(List<int> bytes, int start) {
    final chars = <int>[];
  ...
  static String _readUtf16BE(List<int> bytes, int start) {
    final chars = <int>[];
    for (int i = start; i < bytes.length - 1; i += 2) {
  ...
  static Map<String, String> _parseId3v1(List<int> bytes) {
    final result = <String, String>{};
    final offset = bytes.length - 128;
  ...
  static Future<bool> _checkLrcExists(String audioFilePath) async {
